# xgboost 모듈이 맞는지 확인하여 주세요. (16번째 줄)
# 데이터의 크기를 작게 설정해 놓았어요. (36번째 줄)
# plt.savefig 하기 위한 저장소가 필요합니다. (해당 파일과 같은 위치에 'image' 라는 폴더를 생성하여 주세요.)
...
# Find attribute that has the best correlation value top 9, continuous and categorical each
...
def standardScaler(df, column_list) :
    standard = StandardScaler()
    scaled_df = standard.fit_transform(df.iloc[ : , 1:57])
...
def minmaxScaler(df, column_list) :
    minmax = MinMaxScaler()
    scaled_df = minmax.fit_transform(df.iloc[ : , 1:57])
    scaled_df = pd.DataFrame(scaled_df)
    scaled_df.insert(0, 'Churn', df['Churn'])
    scaled_df.columns = column_list
    return scaled_df
...
def robustScaler(df, column_list) :
    robust = RobustScaler()
    scaled_df = robust.fit_transform(df.iloc[ : , 1:57])
    scaled_df = pd.DataFrame(scaled_df)
    scaled_df.insert(0, 'Churn', df['Churn'])
    scaled_df.columns = column_list
    return scaled_df
...
def buildModel(df, X_train, X_test, y_train, y_test) :
    df_list = ['Gaussian NB', 'Decision Tree', 'Logistic Regression', 'XGBoost']
    df_params = []
    df_score = []
    ...
    xgboost_best_param, xgboost_best_score = xgBoost(X_train, X_test, y_train, y_test)
    df_params.append(xgboost_best_param)
    df_score.append(xgboost_best_score)
    ...
    xgb_clf = XGBClassifier(booster = ['gbtree', 'gblinear', 'dart'], max_depth = [5, 10, 100], gamma = [0, 1, 5, 10])

    # 각 모델의 가장 높은 점수를 가지는 파라미터들을 이용하여 Voting Classifier를 구현합니다.
    soft_voting_model = VotingClassifier(estimators=[ ('GaussianNB', gnb_clf), ('DecisionTree', dt_clf), ('LogisticRegression', lost_reg), ('XGBoost', xgb_clf)],
    ...
    # Soft Voting model을 이용하여 구하여진 값을 0% 부터 100%까지 구간을 나누어 시각화 합니다.
    ...
    # 이 때, Voting classifier는 반영되지 않습니다.
...
def xgBoost (X_train, X_test, y_train, y_test) :
    ...
    parameters = {'booster' : ['gbtree'], 'max_depth' : [5, 6, 8], 'gamma':[0, 1, 2, 3]}
    grid_search = GridSearchCV(estimator = classifier, param_grid = parameters, scoring = 'accuracy')
    grid_search.fit(X_train, y_train)
    best_parameter = grid_search.best_params_
    best_score = round(grid_search.best_score_, 4)
    print('XGBoost Best Parameter: {}'.format(best_parameter))
...
def countChurn(df) :
    ...
    print('No: {}\n'.format(total[0]))
    ax = total.plot(kind='bar', title="Churn Value Count", rot=0, color=['salmon', 'darkslateblue'])
    for p in ax.patches: 
        left, _, width, height = p.get_bbox().bounds 
        ax.annotate("{0}".format(int(height)), (left+width/2, height*1.01), ha='center')
...
def visualDistribution(df, column_list, top_cate, top_cont) :
    ...
    num = 1
    for i in top_cate_columns:
        plt.subplot(3, 3, num)
        num += 1
        count_cal = pd.value_counts(df[i])
        count_cal.plot(kind='bar', rot=0, title = '{}'.format(i))
...
def confusionMatrix(dataset, model, params, X_train, X_test, y_train, y_test) :
    if (model == 'Gaussian NB') :
        ...
        classifier = XGBClassifier(booster = params['booster'], max_depth = params['max_depth'], gamma = ['gamma'])
        classifier.fit(X_train, y_train)
        y_predict = classifier.predict(X_test)
    else :
...
def rocCurve(dataset, model, params, X_train, X_test, y_train, y_test) : 
    ...
    precision_score = (matrix[1][1]) / (matrix[1][1] + matrix[0][1])
    recall_score = (matrix[1][1]) / (matrix[1][1] + matrix[1][0])
    f1score_score = (2 *  precision_score * recall_score) / (precision_score + recall_score) 
    
    print("* Accuracy: {}".format(round(accuracy_score, 4)))
    print("* Precision: {}".format(round(precision_score, 4)))
    print("* Recall: {}".format(round(recall_score, 4)))
    print("* F1 score: {}".format(round(f1score_score, 4)))
